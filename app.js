...
globalThis.startBearApp=() => {
  ...
  const specialSpecs={
    '最好的伙伴':{text:'造成克制伤害后，获得攻防速 +20%',defense:.2},'裁决':{text:'造成克制伤害后，获得攻防速 +20%',defense:.2},'滋养':{text:'造成克制伤害后，获得攻防速 +20%',defense:.2},'点燃':{text:'造成克制伤害后，获得攻防速 +20%',defense:.2},'净化':{text:'造成克制伤害后，获得攻防速 +20%',defense:.2},
    '虫群鼓舞':{text:'每只其他虫系精灵，入场时攻防速 +10%',defense:.1},'虫群突袭':{text:'每只其他虫系精灵，入场时攻防速 +15%',defense:.15},'鼓气':{text:'触发后双攻和双防 +20%',defense:.2},'三鼓作气':{text:'触发后双攻和双防永久 +20%',defense:.2},'合拍':{text:'每个相同项目，物攻和物防永久 +10%',defense:.1},'淬炼火':{text:'每次火系技能，攻防 +10%（最多10次）',defense:.1,max:10},'守护之心':{text:'每种场上增益，物防 +20%',defense:.2},'宇宙之眼':{text:'每层星陨印记，物防 +10%',defense:.1},'蒸汽革命':{text:'每次火系技能，物防 +5%',defense:.05},'囤积':{text:'每有1能量，双防 +10%',defense:.1}
  };
  ...
  function validate(s){
    ...
    out.specialLevel=Math.trunc(E.bounded(s.specialLevel,0,99,0));out.specialEnabled=s.specialEnabled===true;
  ...
  function card(side){
    ...
    if(!atk){
      ...
      const spec=specialSpec(p);if(spec&&p.trait!=='囤积'){const max=spec.max||99;if(starTraits.has(p.trait))$('special-enabled').onchange=e=>{state.specialEnabled=e.target.checked;render();};else{bindNumber('special-level',x=>{state.specialLevel=Math.trunc(x);},0,max);$('special-range').oninput=e=>{state.specialLevel=Number(e.target.value);render();};for(const [id,step] of [['special-minus',-1],['special-plus',1]])$(id).onclick=()=>{state.specialLevel=Math.trunc(E.bounded(state.specialLevel+step,0,max));render(true);};}}
    }
  }
  ...
  function config(){const a=stats('atk'),d=stats('def'),pet=E.pets[state.def.pet],spec=specialSpec(pet),star=starTraits.has(pet.trait),level=spec&&pet.trait!=='囤积'?(star?(state.specialEnabled?1:0):state.specialLevel):0;return {...state,stockpile:pet.trait==='囤积'?state.stockpile:0,traitDefensePct:spec?.defense?spec.defense*level:0,attack:a[1],defense:d[3],types:pet.types,maxHP:d[0],currentHP:state.currentHP===null?d[0]:Math.min(d[0],state.currentHP)};}
  function render(force=false){
    ...
    if($('special-level')){const max=Number($('special-range').max||99);setValue('special-level',state.specialLevel,force);$('special-range').value=state.specialLevel;$('special-range').style.setProperty('--stockpile-fill',state.specialLevel/max*100+'%');$('special-range').classList.toggle('has-level',state.specialLevel!==0);$('special-range').setAttribute('aria-valuetext',`${state.specialLevel} 层，敌方物防 +${state.specialLevel*(Number($('special-range').dataset.pct||10))}%`);$('special-minus').disabled=state.specialLevel===0;$('special-plus').disabled=state.specialLevel===max;}
    if($('special-enabled'))$('special-enabled').checked=state.specialEnabled;
    ...
    $('formula').innerHTML=`<div class="formula-grid"><div><b>① 能力等级倍率</b>(1 + ${Math.max(state.atkLevel,0)/10} 我方物攻提升 + ${Math.max(-state.defLevel,0)/10} 敌方物防降低)<br>÷ (1 + ${Math.max(-state.atkLevel,0)/10} 我方物攻降低 + ${Math.max(state.defLevel,0)/10} 敌方物防提升 + ${r.stockpile/10} 囤积双防提升 + ${r.traitDefensePct} 特性物防)<br>= ${r.abilityNumerator} / ${r.abilityDenominator} ≈ ${Number(r.ability.toFixed(6))}</div><div><b>② 先发制人 · 普通系物理伤害</b>威力 = max(0, 55 × 1 应对倍率 + ${r.powerBonus}) = ${r.skillPower}<br>⌊ (${c.attack} ÷ ${c.defense}) × 37/41 × ${r.skillPower} × (${r.abilityNumerator}/${r.abilityDenominator})<br>× 1 本系 × ${r.mainEffect} 克制 × ${c.mainMult} 其他影响 × ${(100-c.reduction)/100} 减伤剩余 × 1 其他免伤 ⌋ = ${r.main} HP</div><div><b>③ 星陨印记 · 引爆伤害</b>n = min(99, ${state.freeze} 冻结 + ${state.marks} 已有) = ${r.layers}；印记威力 = ${r.power}<br>结算威力 = max(0, ${r.power} + ${r.powerBonus})${r.layers===0?"（零层不触发）":""} = ${r.starPower}<br>⌊ (${c.attack} ÷ ${c.defense}) × 37/41 × ${r.starPower} × (${r.abilityNumerator}/${r.abilityDenominator}) 能力等级 × ${r.starEffect} 克制 × ${c.starMult} 其他影响 × ${(100-c.reduction)/100} 减伤剩余 × 1 其他免伤 ⌋ = ${r.star} HP<br>不乘本系加成；零层印记不触发。</div><div><b>④ 冻结生命线与斩杀合计</b>⌊ ${c.maxHP} × ${state.freeze} / 20 ⌋ = ${r.frozenHP} HP<br>攻击后生命 ≤ 冻结生命线 → 击败<br>斩杀线 = ${r.main} 先发 + ${r.star} 星陨 + ${r.frozenHP} 冻结 = ${r.executionLine} HP。</div></div>`;
  }
