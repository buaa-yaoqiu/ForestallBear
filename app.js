...
globalThis.startBearApp=() => {
  ...
  function render(force=false){
    ...
    if($('special-level')){const max=Number($('special-range').max||99);setValue('special-level',state.specialLevel,force);$('special-range').value=state.specialLevel;$('special-range').style.setProperty('--stockpile-fill',state.specialLevel/max*100+'%');$('special-range').classList.toggle('has-level',state.specialLevel!==0);$('special-minus').disabled=state.specialLevel===0;$('special-plus').disabled=state.specialLevel===max;}
    ...
    $('formula').innerHTML=`<div class="formula-grid"><div><b>① 能力等级倍率</b>(1 + ${Math.max(state.atkLevel,0)/10} 我方物攻提升 + ${Math.max(-state.defLevel,0)/10} 敌方物防降低 + ${r.traitAttackPct} 特性物攻)<br>÷ (1 + ${Math.max(-state.atkLevel,0)/10} 我方物攻降低 + ${Math.max(state.defLevel,0)/10} 敌方物防提升 + ${r.stockpile/10} 囤积双防提升 + ${r.traitDefensePct} 特性物防)<br>= ${r.abilityNumerator} / ${r.abilityDenominator} ≈ ${Number(r.ability.toFixed(6))}</div><div><b>② 先发制人 · 普通系物理伤害</b>威力 = max(0, 55 × 1 应对倍率 + ${r.powerBonus}) = ${r.skillPower}<br>⌊ (${c.attack} ÷ ${c.defense}) × 37/41 × ${r.skillPower} × (${r.abilityNumerator}/${r.abilityDenominator})<br>× 1 本系 × ${r.mainEffect} 克制 × ${c.mainMult} 其他影响 × ${(100-c.reduction)/100} 减伤剩余 × 1 其他免伤 ⌋ = ${r.main} HP</div><div><b>③ 星陨印记 · 引爆伤害</b>n = min(99, ${state.freeze} 冻结 + ${state.marks} 已有) = ${r.layers}；印记威力 = ${r.power}<br>结算威力 = max(0, ${r.power} + ${r.powerBonus})${r.layers===0?"（零层不触发）":""} = ${r.starPower}<br>⌊ (${c.attack} ÷ ${c.defense}) × 37/41 × ${r.starPower} × (${r.abilityNumerator}/${r.abilityDenominator}) 能力等级 × ${r.starEffect} 克制 × ${c.starMult} 其他影响 × ${(100-c.reduction)/100} 减伤剩余 × 1 其他免伤 ⌋ = ${r.star} HP<br>不乘本系加成；零层印记不触发。</div><div><b>④ 冻结生命线与斩杀合计</b>⌊ ${c.maxHP} × ${state.freeze} / 20 ⌋ = ${r.frozenHP} HP<br>攻击后生命 ≤ 冻结生命线 → 击败<br>斩杀线 = ${r.main} 先发 + ${r.star} 星陨 + ${r.frozenHP} 冻结 = ${r.executionLine} HP。</div></div>`;
  }
