...
(() => {
  ...
  function validate(s){
    ...
    for(const side of ['atk','def']){
      ...
      if(!Number.isInteger(v.pet)||!E.pets[v.pet]||(side==='atk'&&v.pet!==0))throw Error('精灵不在测试名单中');
  ...
  function render(force=false){
    ...
    const reaction=kill||!alive?'先发拿下':r.segments.red*20<c.maxHP?'大发雷霆':'冰点你咯';
    ...
    $('needed-note').textContent=!alive?'目标当前生命为 0，请调整血量。':minimum===null?'在 0–20 层冻结范围内，未达到斩杀条件。':r.alreadyFrozen?'当前生命不高于冻结生命线，无需本次攻击即满足死亡条件。':minimum===0?'无需冻结，本体与已有星陨伤害已足够。':`已计入冻结生命线；保留 ${state.marks} 层星陨，${state.freeze>=minimum?'当前层数已达标。':`还需 ${minimum-state.freeze} 层冻结。`}`;
